...
## the input to this should be ${NAME}_clean_r1.fastq.gz -- > however, there is some read name issue when i do that
bwa aln -t 3 ${REF} ${NAME}_clean_r1.fastq.gz > ${NAME}_aln_r1.sai 
bwa aln -t 3 ${REF} ${NAME}_clean_r2.fastq.gz > ${NAME}_aln_r2.sai

## create paired end alignment
bwa sampe ${REF} ${NAME}_aln_r1.sai ${NAME}_aln_r2.sai ${NAME}_clean_r1.fastq.gz ${NAME}_clean_r2.fastq.gz > ${NAME}_aln_pe.sam
